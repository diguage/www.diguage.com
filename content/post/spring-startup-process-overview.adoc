...
.. `ImportAwareBeanPostProcessor` -- 无所事事。
...
.. `AnnotationAwareAspectJAutoProxyCreator` -- 无所事事。
.. `CommonAnnotationBeanPostProcessor` -- 调用 `postProcessProperties` 完成注入，注入工作是在 `InjectionMetadata.InjectedElement` 完成的。这里并没有注入，似乎调用了全局设置的初始化操作。需要找文档确认一下。
.. `AutowiredAnnotationBeanPostProcessor` -- 无所事事。
...
.. `ImportAwareBeanPostProcessor` -- 继承默认实现，什么操作也不做。
