...
学习 Spring 代码，最重要的是学习 Spring 有哪些扩展点，可以利用这些扩展点对 Spring 做哪些扩展操作。说得更具体一点，如果自己开发一个框架，如何与 Spring 进行整合，如果对 Spring 的扩展点有一个比较清晰的认识，势必会事半功倍。
...
. https://blog.csdn.net/qq_36827957/article/details/98725006[Spring Boot 集成Seata(fescar)实现分布式事务处理以及实现机制_java_小老弟的博客-CSDN博客^]
