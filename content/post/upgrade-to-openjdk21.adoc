...
启动项： `--add-opens java.base/java.util=ALL-UNNAMED`。
...
[source%nowrap,{source_attr}]
----
Caused by: java.lang.IllegalAccessError: class org.msgpack.template.TemplateRegistry (in unnamed module @0x21282ed8) cannot access class sun.util.calendar.ZoneInfo (in module java.base) because module java.base does not export sun.util.calendar to unnamed module @0x21282ed8
----

启动项： `--add-opens java.base/sun.util.calendar=ALL-UNNAMED`。
...
启动项： `--add-opens java.base/java.math=ALL-UNNAMED`。
